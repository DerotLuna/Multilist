...
void mostrarP(principal *p)                            //Muestra la lista principal.
{
	principal *aux = p;
	while (aux)
	{
...
void mostrar(lista *p)                    //Muestra toda una sub-lista.
{
	lista *aux = p;
	while (aux)
	{
...
void eliminarYcambiar(principal **p, int x)         Elimina el principal y cambia su sublista a otro principal
{
	int z;
	principal *aux = (*p);
	if (!(vacio(*p)))
